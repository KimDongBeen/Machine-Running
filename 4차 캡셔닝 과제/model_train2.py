...
class vedio2text:
    ...
    def build_model(self, feat, captions=None, cap_len=None, sampling=None, phase=0):
        ...
        with tf.variable_scope('LSTM1'):
            ...
            lstm_red_bw = tf.nn.rnn_cell.BasicLSTMCell(n_hidden, forget_bias=forget_bias_red, state_is_tuple=True)

            if phase == phases['train']:
                lstm_red = tf.contrib.rnn.DropoutWrapper(lstm_red, output_keep_prob=dropout_prob)
                lstm_red_bw = tf.contrib.rnn.DropoutWrapper(lstm_red_bw, output_keep_prob=dropout_prob)
        ...
        '''
        state_red_bw = lstm_red_bw.zero_state(batch_size, dtype=tf.float32)
        ...
        for i in range(0, n_frames):
            with tf.variable_scope("LSTM1"):
                ...
                output_red_bw, state_red_bw = lstm_red_bw(image_emb[i, :, :], state_red_bw)
                output_red_Total = tf.concat([output_red, output_red_bw], axis=1)

            with tf.variable_scope("LSTM2"):
                output_gre, state_gre = lstm_gre(tf.concat([padding, output_red_Total], axis=1), state_gre)
        ...
        for i in range(0, max_caption_len):
            with tf.variable_scope("LSTM1"):
                ...
                output_red_bw, state_red_bw = lstm_red_bw(padding_in, state_red_bw)
                output_red_Total = tf.concat([output_red, output_red_bw], axis=1)

            if i == 0:
                with tf.variable_scope("LSTM2"):
                    con = tf.concat([bos, output_red_Total], axis=1)
                    '''
                    [2]
                    h_src를 2차원으로 변경 후,
                ...
                with tf.variable_scope("LSTM2"):
                    con = tf.concat([embed_result, output_red_Total], axis=1)
                    '''
