...
test_mode = False
learning_rate = 0.001
num_epoches = 50  # 전체 Epoch 횟수
batch_size = 128  # Batch Size 조절
...
본인 경로로 변경
'''
data_dir = 'C:\\Users\\Kim DongBeen\\Desktop\\homework_4-master\\homework_4-master\\data'
test_dir = 'C:\\Users\\Kim DongBeen\\Desktop\\homework_4-master\\homework_4-master\\data\\testing_data'
...
class vedio2text:
    ...
    def build_model(self, feat, captions=None, cap_len=None, sampling=None, phase=0):
        ...
        with tf.variable_scope('LSTM2'):
            ...
            lstm_gre_bw = tf.nn.rnn_cell.BasicLSTMCell(n_hidden, forget_bias=forget_bias_gre, state_is_tuple=True)
            if phase == phases['train']:
                lstm_gre = tf.contrib.rnn.DropoutWrapper(lstm_gre, output_keep_prob=dropout_prob)
                lstm_gre_bw = tf.contrib.rnn.DropoutWrapper(lstm_gre_bw, output_keep_prob=dropout_prob)
        ...
        '''
        state_gre_bw = lstm_gre_bw.zero_state(batch_size, dtype=tf.float32)
        ...
        for i in range(0, n_frames):
            ...
            with tf.variable_scope("LSTM2"):
                ...
                output_gre_bw, state_gre_bw = lstm_gre(tf.concat([padding, output_red], axis=1), state_gre_bw)
                output_gre_Total = tf.concat([output_gre, output_gre_bw], axis=1)
                h_src.append(output_gre_Total)  # even though padding is augmented, output_gre/state_gre's shape not change
        ######################################
        ...
        for i in range(0, max_caption_len):
            ...
            if i == 0:
                with tf.variable_scope("LSTM2"):
                    ...
                    output_gre_bw, state_gre_bw = lstm_gre_bw(con, state_gre_bw)
            else:
                if phase == phases['train']:
                    if sampling[i] == True:
            ...
            logit_words = tf.matmul(tf.concat([output_gre, output_gre_bw], axis=1), weights['W_dec']) + biases['b_dec']
